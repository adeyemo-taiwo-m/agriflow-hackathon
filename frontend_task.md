...
# Farms (Phase 1E)
POST /api/v1/farms/          (Step 1: Core Details)
POST /api/v1/farms/{id}/uploads (Step 2: Photos & Location)
...
The Create Farm tab remains a guided wizard, but the **API submission is a two-step process** triggered once the user clicks "Submit" in Step 4.

**UI Flow:**
- **Step 1 — Details**: Crop selector, Name, State, LGA, Size, Description. (Location capture and photos also happen here in the UI, but are sent in Step 2 of the API).
...
- **Step 4 — Review & Submit**: Review summary and click "Submit".
...
2.  **Step 2: Upload Files & Location**
    ...
    ```
    - On success → Milestones are auto-generated by backend. Redirect to success screen.
