...
function MilestonesTab({ farms: propFarms }) {
  const [farms, setFarms] = useState(propFarms || []);
  const [loading, setLoading] = useState(!propFarms);
  const { addToast } = useToast();
  const { user } = useAuth();
  ...
  const activeFarms = (farms || []).filter(f => !['DRAFT', 'draft'].includes(f.farm_status));
                    ...
                    <div key={farm.id} className="card" style={{padding:'20px 24px', opacity:['cancelled','rejected'].includes(farm.farm_status)?0.7:1}}>
                      <div style={{display:'flex',alignItems:'center',gap:'16px',flexWrap:'wrap'}}>
                        ...
                        <div style={{display:'flex',gap:'8px'}}>
                          <Link to={`/farms/${farm.id}`} className="btn btn-ghost btn-sm">View Listing</Link>
                          
                          {farm.farm_status === 'draft' && (
                            <button className="btn btn-solid btn-sm" onClick={() => { setTab('add'); /* In real app, we'd load draft data */ }}>Continue Submission</button>
                          )}
                          
                          {['active', 'funded'].includes(farm.farm_status) && (
                            <button className="btn btn-solid btn-sm" onClick={() => handleTabChange('milestones')}>Milestones</button>
                          )}
                          
                          {farm.farm_status === 'pending' && (
                            <button className="btn btn-ghost btn-sm" disabled style={{ opacity: 0.5, cursor: 'not-allowed' }}>Milestones Locked</button>
                          )}
                        </div>
                      </div>
                    </div>
